...
let { USER_ID: depUSER_ID, DONEURL, VK_TOKEN, ACCOUNT_DONEURL } = existsFile('./.config.js')? require('./.config.js'): {};
let USER_ID = false;
...
vConinWS.onReceiveDataEvent(async (place, score)=> {
	...
	if(place > 0 && !rl.isQst) {
		...
		if(!transferScore && score > 3e6*3 || transferScore<trsum/(1e3) && (trsum=transferScore)) boosterTTL&&await askDonate(vConinWS, trsum);
	...
	switch(line.trim()) {
		...
		case 'transfer':
			...
			try {
				...
				let template = "Отправили ["+formateSCORE(count*1e3, true)+"] score от vk.com/id"+USER_ID+" для vk.com/i"+id;
				try { await infLog(template); } catch(e) {}
			} catch(e) {
...
for (var argn = 2; argn < process.argv.length; argn++) {
	...
	if([ "-h", "-help", "-f", "-t", "-flog", "-autobuy", "-u", "-tforce", "-to", "-ti", "-tsum", "-autobuyItem", "-o" ].includes(process.argv[argn])) {
		...
		if (process.argv[argn] == "-o") {
			let dTest = process.argv[argn + 1];
			if(typeof dTest == "string" && dTest.length < 3) {
				let num = parseInt(dTest)
				con("Выбран аккаунт номер "+dTest);
				DONEURL = ACCOUNT_DONEURL[num];
			}
		}
		...
		if (process.argv[argn] == "-h" || process.argv[argn] == "-help") {
			...
			ccon("-o [num]	- выбор аккаунта для ACCOUNT_DONEURL")
			process.exit();
			continue;
		}
	}
